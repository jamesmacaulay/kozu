...
var composeArray = behaviors.spreadsArrayArgument(core.compose);

function buildAgnosticWrapper(/* inputTransformers... */) {
  var transformer = composeArray(core.cons(allArrayItems, arguments).concat(allArrayItems));
  return core.compose(
    behaviors.gathersThisAndArguments,
    behaviors.transformsArgumentWith(transformer),
    promises.executesWhenever,
    behaviors.spreadsThisAndArguments
  );
}
...
var buildAgnosticArgumentTransformer = core.compose(
  ...
  behaviors.variadic(core.partial(core.cons, null))
);
...
var applySpecialWrappers = transformers.objectPropertyTemplate({
  ...
  objectTemplate: core.compose(
    ...
    behaviors.transformsReturnValueWith(promises.firstArgumentObjectAgnostic)
  )
});
